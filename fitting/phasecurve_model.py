...
wls, tess_response = np.genfromtxt(os.getcwd() +
...
def phase_integrand(alpha, xi1, xi2):
    return phase_function(alpha, xi1, xi2) * np.sin(abs(alpha))
...
def phase_integral(xi1, xi2):
    ...
    Returns:
        float
    """
    result = integrate.quad(phase_integrand, -np.pi, np.pi)
    return result
